...
package body Spica.Splay_Trees is
   ...
   -- Search the subtree rooted at N for Item. Return a pointer to the item's
   -- node if it is found, and the null pointer otherwise. If given a null
   -- node, this function returns null.
   --
   function Basic_Lookup
     (N : Node_Access; Item : Item_Type) return Node_Access is
      X : Node_Access := N;
   begin
      while X /= null loop
         if Item < X.Item then
            X := X.Left;
         elsif X.Item < Item then
            X := X.Right;
         else
            exit;
         end if;
   ...
   procedure Basic_Insert
     (T : in out Tree; Item : Item_Type; Result : out Node_Access) is
   ...
   procedure Insert(T : in out Tree; Item : in Item_Type) is
      N : Node_Access;
   begin
      if T.Root = null then
         T.Root      := new Node;
   ...
   procedure Lookup
     (T : in out Tree; Item : in Item_Type; Result : out Item_Type) is
      N : Node_Access;
   begin
      N := Basic_Lookup(T.Root, Item);
      if N = null then raise Not_Found; end if;
      Adjust_Root(T, N);
      Result := T.Root.Item;
   end Lookup;
   ...
   procedure Check_Consistency(T : in Tree) is
      ...
      begin
         ...
         if N.Left /= null then
            if not (N.Left.Item < N.Item) then raise Inconsistent; end if;
            if N.Left.Parent /= N then raise Inconsistent; end if;
            Traverse(N.Left);
         end if;
         if N.Right /= null then
            if not (N.Item < N.Right.Item) then raise Inconsistent; end if;
            if N.Right.Parent /= N then raise Inconsistent; end if;
            Traverse(N.Right);
         end if;
   ...
   begin
      ...
      if T.Root  = null and T.Count /= 0 then raise Inconsistent; end if;
      if T.Root.Parent /= null then raise Inconsistent; end if;
      Traverse(T.Root);
      if Actual_Count /= T.Count then raise Inconsistent; end if;
