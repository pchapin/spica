...
package body Spica.Graphs is
   ...
   procedure Create_Vertex
     (The_Graph     : in out Graph;
      ...
      Vertex_Number : out    Vertex_Index) is

      Empty_EdgesF : Edge_Tables.Vector;
      Empty_EdgesB : Edge_Tables.Vector;
   begin
   ...
   procedure Create_Edge
     (The_Graph : in out Graph;
      ...
      To        : in     Vertex_Index) is

      function Edge_Exists return Boolean is
         Count : Natural := The_Graph.Forward.Element(From).Last_Index;
      begin
         for I in 1 .. Count loop
            if The_Graph.Forward.Element(From).Element(I) = To then
               return True;
            end if;
         end loop;
         return False;
      ...
      procedure Append_To(Edge_Vector : in out Edge_Tables.Vector) is
      begin
         Edge_Vector.Append(To);
      end Append_To;

      -- Helper procedure used in call to Update_Element.
      procedure Append_From(Edge_Vector : in out Edge_Tables.Vector) is
      begin
   ...
   function Get_Vertex
     (The_Graph : Graph; Vertex_Number : Vertex_Index) return Vertex_Type is
   begin
   ...
   function Get_Successor_List
     (The_Graph : Graph; Vertex_Number : Vertex_Index) return Vertex_List is
   begin
      if Vertex_Number > The_Graph.Size then raise Bad_Vertex; end if;
      declare
   ...
   function Get_Predecessor_List
     (The_Graph : Graph; Vertex_Number : Vertex_Index)
